...
import { Plus, Trash2, Edit2, Tag, Search, X } from "lucide-react";
...
const initialCategories = [
  {
    ...
    name: "Travel Tips",
    count: 4,
    description: "Essential advice for travelers.",
  },
  {
    id: 2,
    name: "Food & Culture",
    count: 8,
    description: "Culinary journeys and traditions.",
  },
  {
    id: 3,
    name: "City Guides",
    count: 12,
    description: "In-depth guides to Moroccan cities.",
  },
  {
    id: 4,
    name: "Adventure",
    count: 6,
    description: "Activities for the thrill seekers.",
  },
];
...
export default function BlogCategoriesPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (window.innerWidth < 1024) setIsMobileFormOpen(false); // Close on mobile after submit
  };

  const startEdit = (cat: (typeof initialCategories)[0]) => {
    setIsEditing(cat.id);
    setFormData({ name: cat.name, description: cat.description });
    setIsMobileFormOpen(true); // Open form on mobile when editing
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          onClick={() => {
            ...
            if (isEditing && isMobileFormOpen) cancelEdit();
          }}
                ...
                filteredCategories.map((cat) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${isEditing === cat.id ? "bg-emerald-500 text-white" : "bg-indigo-50 text-indigo-600"} transition-colors`}
                      >
